...
def get_price_for_buy(ticker_price, bot_config, db_price_data_obj):
    ...
    if max_price_data['count'] < (price_range_minutes * 10):
        custom_logger.write_log('Not enough price data, count: ' + str(max_price_data['count']))
        return None
    ...
    if current_price < max_price and current_price < float(ticker_price['highPrice']):
        ...
        custom_logger.write_log(message.format(max_price, current_price, price_diff_percent, price_diff_24hr_high_percent))
